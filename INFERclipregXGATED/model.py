...
def build_model(state_dict: dict, regtoken_path="regtokens"):
    ...
    #state_dict["visual.positional_embedding"]= nn.Parameter(torch.randn(261, 1024) * 0.5)
    ...
    new_pos_embed = state_dict["visual.positional_embedding"].shape[0] # Verify model is instantiated correctly
    print("Checking model parameters... You should see [261, _ ]. If you see [257, _ ], something is wrong.")
    print(f"vision_width, vision_layers, patch_size, grid_size:, new_pos_embed: {vision_width}, {vision_layers}, {vision_patch_size}, {grid_size}, {new_pos_embed}")
    if new_pos_embed != 261:
        # Adjust state_dict to include [REG] tokens ONLY when not existent - for example, loading .safetensors
