...
def convert_pickle_to_safetensors(input_path, output_path=None):
    ...
    # **Ensure `visual.register_tokens` is explicitly stored!**
    if "visual.register_tokens" not in state_dict:
        raise ValueError("[!!! ERROR !!!] `visual.register_tokens` is missing! Something is wrong!")

    print("[---! OK !---] `visual.register_tokens` is retained.")

    # **Ensure positional embeddings are stored correctly!**
    if "visual.positional_embedding" not in state_dict:
        raise ValueError("[!!! ERROR !!!] `visual.positional_embedding` is missing! Something is wrong!")

    print("[---! OK !---] `visual.positional_embedding` is retained.")
