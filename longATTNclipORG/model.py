...
"""
### END Attention visualization code

# model.py follows below 
"""
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        batch_size, seq_len, dim = x.shape  # Get batch size dynamically

        if self.attn_mask is not None:
            n_ctx = x.shape[0]
            self.attn_mask = self.attn_mask[..., -n_ctx:, -n_ctx:].to(dtype=x.dtype, device=x.device)            
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.context_length = 248 # Long-CLIP
    ...
    def encode_text(self, text):
        ...
        # ---- Long:
        x = x + (self.positional_embedding[:n_ctx].to(x.device) * self.mask1[:n_ctx].to(x.device)).type(self.dtype).to(x.device) + (self.positional_embedding_res[:n_ctx].to(x.device) * self.mask2[:n_ctx].to(x.device)).type(self.dtype).to(x.device) 
