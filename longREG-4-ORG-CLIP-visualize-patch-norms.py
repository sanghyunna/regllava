...
from longmodel import longclip as clip
from longmodel.model_longclip import CLIP

device = "cuda" if torch.cuda.is_available() else "cpu"

def parse_arguments():
    parser = argparse.ArgumentParser(description='Visualize Long-CLIP ViT Patch Norms')
...
out_dir = "plots/Long-CLIP/patch-norm"
...
def clip_encode_image(modelorg, image_input):
    with torch.no_grad():
        ...
        cls_token_post = model.visual.ln_post(x[:, 0, :])
        patch_tokens = x[:, 1:, :]
        
        
        norm_cls = torch.norm(cls_token_post, dim=-1).squeeze(0) 
        norm_patch = torch.norm(patch_tokens, dim=-1).squeeze(0)
        
        combined_norms = torch.cat([norm_patch, norm_cls.unsqueeze(0)], dim=0)
...
for img_file in tqdm(os.listdir(img_dir)):
    if img_file.lower().endswith((".png", ".jpg", ".jpeg")):
        ...
        #token = tokens[0] # don't need this for just patch norm
